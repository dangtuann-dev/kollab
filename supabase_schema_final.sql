-- ============================================================
--  Kollab - Agile/Scrum Project Management System
--  Database Schema (Tổng hợp - Phiên bản hoàn chỉnh)
--  Đồ án cơ sở - Đặng Thanh Tuấn - MSSV 240000794
--
--  Hướng dẫn: Chạy toàn bộ script này trong Supabase SQL Editor.
--  Script có thể chạy lại nhiều lần (idempotent).
-- ============================================================

--------------------------------------------------------------------------------
-- BƯỚC 0: Dọn dẹp toàn bộ objects cũ (theo thứ tự ngược dependency)
--------------------------------------------------------------------------------
...
DROP TRIGGER IF EXISTS update_sprints_modtime       ON public.sprints;
DROP TRIGGER IF EXISTS update_user_stories_modtime  ON public.user_stories;
...
-- Drop tables (thứ tự ngược dependency)
...
-- Backfill: tạo profile cho những user đã tồn tại trước khi script được chạy
INSERT INTO public.profiles (id, full_name, email, avatar_url)
SELECT
    id,
...
--------------------------------------------------------------------------------
-- HELPER FUNCTIONS (dùng trong RLS policies)
--------------------------------------------------------------------------------
...
-- Ràng buộc: mỗi project chỉ có 1 sprint đang active tại một thời điểm
CREATE UNIQUE INDEX unique_active_sprint ON public.sprints (project_id)
WHERE (status = 'active');

--------------------------------------------------------------------------------
-- BẢNG 5: user_stories (Product Backlog Items)
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- BẢNG 6: tasks (sub-tasks của user_story)
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- BẢNG 7: comments (trên user_story hoặc task)
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- BẢNG 8: standup_logs (Daily Standup)
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- INDEXES: Hiệu năng & Full-Text Search
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- TRIGGERS: Thông báo phân công task
--------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
-- TRIGGERS: Thông báo bình luận mới
--------------------------------------------------------------------------------
...
BEGIN
    IF new.task_id IS NOT NULL THEN
        SELECT assignee_id, title,
               (SELECT project_id FROM public.user_stories WHERE id = user_story_id)
...
--------------------------------------------------------------------------------
-- TRIGGERS: Ghi log thay đổi task (activity log)
--------------------------------------------------------------------------------
...
BEGIN
    ...
    IF old.title IS DISTINCT FROM new.title OR
       ...
       old.deadline IS DISTINCT FROM new.deadline THEN
        ...
        INSERT INTO public.activity_logs (task_id, user_id, action, old_value, new_value)
        VALUES (
            new.id, v_user_id, 'field_updated',
            jsonb_build_object(
                'title', old.title, 'description', old.description,
                'estimate_hours', old.estimate_hours, 'actual_hours', old.actual_hours,
                'story_points', old.story_points, 'priority', old.priority,
                'labels', old.labels, 'deadline', old.deadline
            ),
            jsonb_build_object(
                'title', new.title, 'description', new.description,
                'estimate_hours', new.estimate_hours, 'actual_hours', new.actual_hours,
                'story_points', new.story_points, 'priority', new.priority,
                'labels', new.labels, 'deadline', new.deadline
            )
        );
    END IF;
    ...
    IF new.task_id IS NOT NULL THEN
        ...
        VALUES (new.task_id, new.user_id, 'commented',
                jsonb_build_object('comment_id', new.id,
                                   'content', substring(new.content from 1 for 50)));
    END IF;
    RETURN new;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
CREATE POLICY "Allow members to view user_stories"
    ...
    USING (auth.role() = 'authenticated' AND public.is_project_member(project_id, auth.uid()));
CREATE POLICY "Allow PO to insert user_stories"
...
CREATE POLICY "Allow members to view comments"
    ...
    USING (auth.role() = 'authenticated' AND (
        (user_story_id IS NOT NULL AND EXISTS (
            SELECT 1 FROM public.user_stories s
            WHERE s.id = user_story_id AND public.is_project_member(s.project_id, auth.uid())
        )) OR
        (task_id IS NOT NULL AND EXISTS (
            SELECT 1 FROM public.tasks t
            JOIN public.user_stories s ON t.user_story_id = s.id
            WHERE t.id = task_id AND public.is_project_member(s.project_id, auth.uid())
        ))
    ));
...
CREATE POLICY "Allow author or PO to delete comments"
    ...
    USING (auth.role() = 'authenticated' AND (
        ...
        EXISTS (
            ...
            WHERE s.id = user_story_id AND (
                p.owner_id = auth.uid() OR
                public.get_project_role(p.id, auth.uid()) = 'product_owner'
            )
        )
    ));
...
-- Tìm kiếm toàn cục (user stories, tasks, projects) trong một project
...
BEGIN
    ...
    SELECT
        ...
        p.description,
        p.id,
        jsonb_build_object('color', p.color)
    ...
    SELECT
        ...
        us.description,
        us.project_id,
        jsonb_build_object('priority', us.priority, 'status', us.status, 'story_points', us.story_points)
    FROM public.user_stories us
    WHERE us.project_id = p_project_id
      AND (us.fts @@ to_tsquery('simple', search_query) OR us.title ILIKE '%' || search_query || '%')
    ...
    SELECT
        ...
        t.description,
        us.project_id,
        jsonb_build_object('status', t.status, 'story_title', us.title)
    FROM public.tasks t
    JOIN public.user_stories us ON t.user_story_id = us.id
    WHERE us.project_id = p_project_id
      AND (t.fts @@ to_tsquery('simple', search_query) OR t.title ILIKE '%' || search_query || '%');
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;


-- Tạo dữ liệu Burndown Chart cho một Sprint
...
DECLARE
    ...
    v_decrement    numeric;
BEGIN
    -- Lấy ngày bắt đầu và kết thúc Sprint
